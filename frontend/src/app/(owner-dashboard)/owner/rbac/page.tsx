...
export default function OwnerRbacPage() {
  ...
  const createUserMutation = useMutation({
    mutationFn: async () => {
      const res = await apiClient.post('/roles/users/create-with-role', {
        ...form,
    ...
    onSuccess: () => {
      ...
      setForm({ username: '', email: '', password: '', first_name: '', last_name: '', role: 'manager' });
    },
  });

  const removeRoleMutation = useMutation({
    mutationFn: async ({ userId, roleId, domain }: { userId: string; roleId: string; domain: string }) => {
      await apiClient.delete('/roles/users/remove-role', {
          ...
          {showCreateUser && (
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              {createUserMutation.isSuccess && (
                <p className="mt-2 text-sm text-green-600">Member created successfully!</p>
              )}
            </div>
          )}
        </>
