...
import {
  MapPin, Star, Search, Heart, SlidersHorizontal,
  Clock, CheckCircle, Map, List, Navigation, Loader2,
} from 'lucide-react';
...
function GroundCard({ ground, isFav, onToggleFav }: {
  ground: FutsalGround;
  isFav: boolean;
  onToggleFav: () => void;
}) {
  ...
  return (
    <Card className="overflow-hidden hover:shadow-xl transition-all duration-200 group flex flex-col h-full border-gray-100">
...
export default function BrowseGroundsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Browse Grounds</h1>
        <p className="text-sm text-gray-500 mt-1">
          {allGrounds.length > 0
            ? `${allGrounds.length} ground${allGrounds.length !== 1 ? 's' : ''} available`
            : 'Find and book the perfect futsal ground near you.'}
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 p-3">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-9 border-0 bg-gray-50 focus-visible:ring-1 focus-visible:ring-emerald-500 h-10"
            />
          </div>
          <button
        ...
        {showFilters && (
          <div className="border-t border-gray-100 px-3 pb-3 pt-3 flex flex-wrap gap-4 bg-gray-50/50">
            ...
            {hasActiveFilters && (
              <div className="flex flex-col gap-1 justify-end">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-sm text-red-500 border border-red-100 bg-white hover:bg-red-50 transition-colors"
                >
                  Clear filters
                </button>
              </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {viewMode === 'map' && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-colors"
            >
              {locState === 'loading' ? (
      ...
      {viewMode === 'map' && !isLoading && (
        <div className="space-y-3">
          ...
          {allGrounds.filter((g) => g.latitude != null && g.longitude != null).length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 rounded-xl bg-gray-100 border border-gray-200 text-gray-400 gap-2">
        ...
        <>
          ...
          {!isLoading && allGrounds.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-lg font-semibold text-gray-700">No grounds found</p>
